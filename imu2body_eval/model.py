...
class IMU2BodyModel(nn.Module):
    def __init__(self, data_config, model_config):
        ...
        if self.wavelet_embedding:
            self.waveletdecomp = WaveletEmbedding(d_channel = 40, swt = True)
            self.waveletrecon = WaveletEmbedding(d_channel = 40, swt = False)
            self.m = 2
        
        if self.use_sep_encoder:
            hand2body_input_dim = (input_dim * 3, mid_dim * 3)
        else:
            hand2body_input_dim = input_dim + mid_dim
    ...
    def forward(self, input_seq, input_img = None, input_pc = None):
        if self.wavelet_embedding:
            input_embedding = self.waveletdecomp(input_seq)
