...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, y, xpos, ypos, xmask, ymask, self_mask):
        x_, self_attn = self.attn(self.norm1(x), xpos, xmask=self_mask, return_attn=True) # f1 f2
        x = x + self.drop_path(x_)
        y_ = self.norm_y(y)
        cross_attn_output, cross_attn = self.cross_attn(self.norm2(x), y_, y_, xpos, ypos, qmask=xmask, kmask=ymask) # f2 f1
