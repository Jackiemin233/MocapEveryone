...
class RGBFiles_Masks(MonocularDataset):
    def __init__(self, dataset_path, masks):
        super().__init__()
        self.use_calibration = False
        self.dataset_path = pathlib.Path(dataset_path)
        self.rgb_files = natsorted(list((self.dataset_path).glob("*.png")))
...
class Intrinsics:
    ...
    def from_calib(img_size, W, H, calib, always_undistort=False):
        ...
        K_opt, _ = cv2.getOptimalNewCameraMatrix(
            K, distortion, (W, H), 0, (W, H), centerPrincipalPoint=center #NOTE: True NJ
        )
...
def load_dataset(dataset_path, masks = None):
    ...
    if masks != None:
        return RGBFiles_Masks(dataset_path, masks)
