import sys
sys.path.insert(0, 'libraries/DROID-SLAM/droid_slam')
sys.path.insert(0, 'libraries/DROID-SLAM')
...
def run_smpl_metric_slam_mast3r(traj, traj_full, pc_whole, pc, kf_idx, smpls=None, smpl_path=None):
    '''
        TODO: no descriptions here, maybe I will write later.
    '''
    cam_R, cam_T = compute_scales(traj, traj_full, pc_whole, pc, kf_idx, smpls=smpls, smpl_path=smpl_path)
    

    return cam_R, cam_T


def run_smpl_metric_slam(img_folder, masks, calib, is_static, smpls=None):
    '''
    Input:
    ...
    # trimesh.Trimesh(smpls[0]['pred_verts'][0], smpls[0]['smpl_faces'])
    ...
    ##### Masked droid slam #####
    droid, traj = run_slam_hybrid(img_folder, masks=masks, calib=calib)
    ...
    ##### Compute the scale - NJ  Work in progress
    droid.compute_scale(smpls, traj)
...
def run_slam_hybrid(imagedir, masks=None, calib=None, depth=None):
    """ Maksed DROID-SLAM """
    droid = None
    if calib is None:
        calib = est_calib(imagedir)
    ...
    for (t, image, intrinsics) in tqdm(image_stream(imagedir, calib)):
        ...
        if droid is None:
            slam_args.image_size = [image.shape[2], image.shape[3]]
            droid = Droid(slam_args) # threshold = 2.4 "how much motion before considering new keyframe"
        
        if masks is not None:
            img_msk = img_msks[t]
            conf_msk = conf_msks[t]
            masked_image = image * (img_msk < 0.5) #cv2.imwrite('/hpc2hdd/home/gzhang292/nanjie/project4/tram/vis/maskedimage.png', image[0].permute(1,2,0).numpy())
            droid.track(t, masked_image, image = None, intrinsics=intrinsics, depth=depth, mask=conf_msk)
...
def test_slam(imagedir, masks, calib, stride=10, max_frame=50):
    ...
    args.stereo = False
    args.upsample = False
    args.disable_vis = True # NOTE NJ: False
    ...
    for (t, image, intrinsics) in image_stream(imagedir, calib, stride, max_frame):
        ...
        if masks is not None:
            img_msk = masks[0][t]
            conf_msk = masks[1][t]
            masked_image = image * (img_msk < 0.5)
            droid.track(t, masked_image, image=None, intrinsics=intrinsics, mask=conf_msk)  
        else:
            droid.track(t, masked_image, image=None,intrinsics=intrinsics, mask=None)  
