...
# vposer, _ = load_model('../data/smpl_models/V02_05', model_code=VPoser, remove_words_in_model_weights='vp_model.', disable_grad=True)
# vposer = vposer.to(device)

from human_body_prior.tools.model_loader import load_vposer
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
vposer, _ = load_vposer('../data/smpl_models/vposer_v1_0')
vposer = vposer.to(device)
...
def create_motion_from_gimo_data(filename, bm, transform_info = None, transform_norm = None, skel_with_offset=None, default_beta=True, load_motion=True):        
	try:
		...
		with torch.no_grad():
			...
			transform_pose = transform_norm @ trans_pose2scene # W_n @ Wp2s
			...
			for i in range(n):
				...
				R_s = torch.from_numpy(Rotation.from_matrix((transform_pose[:3, :3] @ R)).as_rotvec())
				trans_s = torch.from_numpy((transform_pose[:3, :3] @ trans + transform_pose[:3, 3:]).reshape(3))
			...
			pose_body = torch.cat([vposer.decode(np.load(f, allow_pickle=True)['latent'].to(device),\
								output_type='aa') for f in filename], dim = 0).view(n, -1).cpu()
	...
	for frame in range(pose_body.shape[0]):
		...
		for j in range(num_joints):
			if j == 0:
				T = conversions.Rp2T(
					conversions.A2R(root_orient_frame), root_trans_frame # NOTE: print(root_trans_frame)
				)
			else:
				T = conversions.R2T(
					conversions.A2R(
...
# 			n = len(filename)
# 			bdata = {}
# 			bdata['orient'] = torch.cat([np.load(f, allow_pickle=True)['orient'].unsqueeze(0) for f in filename], dim = 0)
# 			bdata['trans'] = torch.cat([np.load(f, allow_pickle=True)['trans'].unsqueeze(0) for f in filename], dim = 0)
# 			pose_body = torch.cat([vposer.decode(np.load(f, allow_pickle=True)['latent'].to(device),\
# 								output_type='aa') for f in filename], dim = 0).view(n, -1).cpu()
  
# 		# with torch.no_grad():
# 		# 	bdata = {}
# 		# 	bdata['orient'] = np.load(filename, allow_pickle=True)['orient'].unsqueeze(0).detach()
# 		# 	bdata['trans'] = np.load(filename, allow_pickle=True)['trans'].unsqueeze(0).detach()
# 		# 	pose_body = vposer.decode(np.load(filename, allow_pickle=True)['latent'].to(device),output_type='aa').view(1, -1).detach().cpu()
