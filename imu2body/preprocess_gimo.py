...
# def load_data_from_gimo(base_dir, file_list, debug=False, start_end=None, transform_info = None, Normalization = True):
...
# 	transform_info = json.load(open(os.path.join(base_dir, '../', transform_info), 'r')) # pose to scene transformation
# 	transform_norm = np.loadtxt(os.path.join(base_dir, '../..', 'scene_obj', 'transform_norm.txt')).reshape((4, 4))
...
# 		motion_list = [gimo.create_motion_from_gimo_data(pkl_files, 
#                                                    bm=body_model, 
...
# 	upvec_axis[2] = 1.0 # TODO: for GIMO the upvec_axis is Z, need interface to adapt different data
...
# 			g_t = head_invert @ global_T[:,i:i+1,...] # normalization?
...
# def load_data_with_args(fnames, bvh_list, start_end ,transform_info, args):
# 	logging.info(f"Start processing {fnames} data with {len(bvh_list)} files...")
# 	data, total_len = load_data(bvh_list, base_dir=os.path.join(args.base_dir, fnames, 'smplx_local'), start_end = start_end, transform_info = transform_info)
...
# def load_data(file_list, base_dir="", start_end = None, transform_info = None):

# 	head_imu_input, ee_pos, output, global_p, local_rot, head_start, c_lr, start_end = load_data_from_gimo(base_dir, file_list, start_end = start_end, transform_info = transform_info)
...
# 	data_info = pd.read_csv(os.path.join(base_dir, 'dataset.csv'))	
# 	# Aligned
# 	fnames_list = (data_info['scene'].astype(str) + '/' + data_info['sequence_path'].astype(str)).tolist()
# 	start_end_list = list(zip(data_info['start_frame'].astype(int), data_info['end_frame'].astype(int)))
# 	transform_info = data_info['transformation']
# 	for fnames, start_end, transform in tqdm(zip(fnames_list, start_end_list, transform_info)):
...
# 		load_data_with_args(fnames=fnames, bvh_list=bvh_list, start_end = start_end, transform_info = transform, args=args)
