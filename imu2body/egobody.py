...
def create_motion_from_egobody_data(filename, bm, transform_info = None, transform_norm = None, skel_with_offset=None, default_beta=True, load_motion=True):        
	try:
		...
		with torch.no_grad():
			...
			for i in range(n):
				...
				ori = np.load(f, allow_pickle=True)['global_orient']
				trans = np.load(f, allow_pickle=True)['transl']   
				body_pose = np.load(f, allow_pickle=True)['body_pose']    
    
				ori_list.append(torch.from_numpy(ori))
				trans_list.append(torch.from_numpy(trans))
				body_pose_list.append(torch.from_numpy(body_pose))
    
			bdata['orient'] = torch.cat(ori_list, dim = 0)
			bdata['trans'] = torch.cat(trans_list, dim = 0)
			pose_body = torch.cat(body_pose_list, dim = 0)
